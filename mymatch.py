    ...
    for epoch in range(epoch + 1, cfg['epochs']):
        ...
        for i, ((img_x, mask_x),
            ...
            with torch.no_grad():
                ...
                fp = model(img_u_w, False, False, True)
                target_shape = (12, 256, 256)

                feature_map = fp[3]
                feature_map = (feature_map - feature_map.min())/(feature_map.max() - feature_map.min())
                feature_map = F.interpolate(feature_map.unsqueeze(1), size=(target_shape[1], target_shape[2]),
                                              mode='bilinear', align_corners=False).squeeze(1)

                for j in range(0,1,2):
                    cur = fp[j]
                    cur = (cur - cur.min())/(cur.max() - cur.min())
                    cur = F.interpolate(cur.unsqueeze(1), size=(target_shape[1], target_shape[2]),
                                              mode='bilinear', align_corners=False).squeeze(1)
                    feature_map += cur
            '''
            ...
            '''
            mask_s1 = torch.where(feature_map > 0.3, 1, 0)
            
            mask_s2 = torch.where(feature_map > 0.1, 1, 0)
            
            img_s1 = img_u_w.clone().cuda()
            img_s2 = img_u_w.clone().cuda()
            
            #img_s1 = mask_s1 * img_u_w;
            
            #img_s2 = mask_s2 * img_u_w;
            
            img_s1[mask_s1.unsqueeze(1).expand(img_s1.shape) == 1] = \
                img_u_s1_mix[mask_s1.unsqueeze(1).expand(img_s1.shape) == 1]
            img_s2[mask_s2.unsqueeze(1).expand(img_s2.shape) == 1] = \
                img_u_s2_mix[mask_s2.unsqueeze(1).expand(img_s2.shape) == 1]
            
            #import ipdb
            #ipdb.set_trace()
            iters = epoch * len(trainloader_u) + i
            kr = 1 - 1 * (1 - iters / total_iters) ** 0.9
            
            model.train()

            num_lb, num_ulb = img_x.shape[0], img_u_w.shape[0]

            preds, preds_fp = model(torch.cat((img_x, img_u_w)), False, False, False, True, kr).chunk(2)
            #import ipdb
            #ipdb.set_trace()
            pred_x, pred_u_w = preds.split([num_lb, num_ulb])
            pred_u_w_fp = preds_fp[num_lb:]

            pred_u_s1, pred_u_s2 = model(torch.cat((img_s1, img_s2)),False, False, False, False).chunk(2)
            ...
            '''
            gt_s1, conf_s1 = mask_u_w.clone(), conf_u_w.clone()
            gt_s2, conf_s2 = mask_u_w.clone(), conf_u_w.clone()

            gt_s1[mask_s1 == 1] = mask_u_w_mix[mask_s1 == 1]
            conf_s1[mask_s1 == 1] = conf_u_w_mix[mask_s1 == 1]

            gt_s2[mask_s2 == 1] = mask_u_w_mix[mask_s2 == 1]
            conf_s2[mask_s2 == 1] = conf_u_w_mix[mask_s2 == 1]
                        
            '''
            feature, feature_s1 ,feature_s2= model(torch.cat((img_u_w,img_s1,img_s2)), False, True).chunk(3)

            loss_fp_s1 = 1 - F.cosine_similarity(feature.view(-1,256*16*16) , feature_s1.view(-1,256*16*16) )
            
            loss_fp_s1 = torch.mean(loss_fp_s1.sum())
            
            loss_fp_s2 = 1 - F.cosine_similarity(feature.view(-1,256*16*16) , feature_s2.view(-1,256*16*16) )
            
            loss_fp_s2 = torch.mean(loss_fp_s2.sum())
            
            loss_fp = ( loss_fp_s1 + loss_fp_s2 )/2.0
            '''
            ...
            loss_u_s1 = criterion_dice(pred_u_s1.softmax(dim=1), gt_s1.unsqueeze(1).float(),
                                       ignore=(conf_s1.squeeze(1) < cfg['conf_thresh']).float())            
            ...
            loss_u_s2 = criterion_dice(pred_u_s2.softmax(dim=1), gt_s2.unsqueeze(1).float(),
                                       ignore=(conf_s2.squeeze(1) < cfg['conf_thresh']).float())            
